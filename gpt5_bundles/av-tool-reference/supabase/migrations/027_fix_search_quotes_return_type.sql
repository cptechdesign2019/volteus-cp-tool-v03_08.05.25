-- ===================================================================
-- Migration: Fix Search Quotes Function Return Type
-- Description: Fix the return type mismatch in search_quotes function
-- Version: 027
-- ===================================================================

-- Drop and recreate the search_quotes function with correct return types
DROP FUNCTION IF EXISTS search_quotes;
...
BEGIN
  ...
  SELECT 
    q.id,
    q.quote_number,
    COALESCE(cc.contact_name, 'Unknown Contact')::TEXT,
    COALESCE(ca.company_name, 'Unknown Company')::TEXT,
    'Sales Rep'::TEXT,  -- Placeholder for now
    q.status,
    q.total_amount,
    q.gross_profit,
